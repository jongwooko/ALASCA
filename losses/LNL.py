...
"""
Code from https://github.com/hitcszx/ALFs/blob/master/losses.py
"""
...
class SCELoss(nn.Module):
    def __init__(self, num_classes=10, a=1, b=1, smooth=1.0):
    ...
    def forward(self, pred, labels, index):
        # Label Smoothing
        label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
        label_one_hot = torch.clamp(label_one_hot, min=max(1e-4, (1 - self.smooth)/self.num_classes), 
                                    max=min(1.0, self.smooth * (1 + 1/self.num_classes)))
        
        # CE
        ce = -torch.mean(torch.sum(torch.log_softmax(pred, dim=1) * label_one_hot, dim=1))
